...
def get_kpi(warehouse_id: str) -> dict:
    conn = get_conn()
    wh = conn.execute("SELECT name, location, capacity FROM warehouses WHERE id=?", (warehouse_id,)).fetchone()
    if not wh:
        conn.close()
        return {"error": f"找不到倉庫 {warehouse_id}"}

    total_claims = conn.execute(
        "SELECT COUNT(*) FROM claims WHERE source_wh=? OR target_wh=?",
        (warehouse_id, warehouse_id)
    ).fetchone()[0]
    open_claims = conn.execute(
        "SELECT COUNT(*) FROM claims WHERE (source_wh=? OR target_wh=?) AND status IN ('待審查','異議中')",
        (warehouse_id, warehouse_id)
    ).fetchone()[0]
    completed_transfers = conn.execute(
        "SELECT COUNT(*) FROM transfer_records WHERE (from_wh=? OR to_wh=?) AND status='已完成'",
        (warehouse_id, warehouse_id)
    ).fetchone()[0]
    conn.close()
...
def get_anomalies() -> dict:
    ...
    in_transit = conn.execute(
        """SELECT t.id, t.from_wh, wf.name as from_name, t.to_wh, wt.name as to_name,
           ...
           WHERE t.status != '已完成' ORDER BY t.created_at"""
    ).fetchall()
    conn.close()

    low_stock = check_safety_stock()
    open_claims = list_claims(status="待審查") + list_claims(status="異議中")
    return {
